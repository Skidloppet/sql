...
<div class="w3-container w3-orange w3-section">
  ...
  <div class="w3-row-padding" style="margin:0 -16px">
        ...
        foreach($pdo->query( 'SELECT * FROM wo;' ) as $row){
          ...
          <td>
            <form action='<?php $_PHP_SELF ?>' method='POST'>
              <input type="hidden" name="deleteWO" value="<?php echo $row['orderID']; ?>">
              <input class="HoverButton" type="submit" name="deleteWO" value="Radera arbetsorder">
            </form>
          </td>
          <?php          
          echo "</tr>";  
    ...
    if(isset($_POST['deleteWO'])){
      $deletedWO = $_POST['deleteWO'];
      $sql = "DELETE FROM WorkOrder WHERE orderID = $deletedWO" ;
      $sql = "DELETE FROM SubPlaceWorkOrder WHERE orderID = $deletedWO" ;
